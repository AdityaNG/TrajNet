...
def random_flip(prediction, target, mask):
...
class ScaleAndShiftInvariantLoss(nn.Module):
    ...
    def forward(self, prediction, target, mask):
        scale = torch.ones((target.shape[0],)).to(target.device)
        shift = torch.zeros((target.shape[0],)).to(target.device)
        if self.__random_flip:
            scale, shift = random_flip(prediction, target, mask)
